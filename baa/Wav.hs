...
put_music file_name music =
  Wave.putWAVEFile
    file_name
    (Wave.WAVE waveHeader (makeSamples (toNotes music)))
...
makeSamples notes =
  map
    ((\x -> [x]) . Wave.doubleToSample)
    (concat (map (flip (sinit 1) 200) (listToPitch (map fromIntegral notes))))
...
sinit' vol n len =
  fadedVols $ qs (n, normvol) $ repeating normlength $ map (sin) values {- map (*normvol) $ -}
  where
    normlength = (len * 110) `div` (length values)
    normvol = (1 / (f2 n)) * vol
